...
const { setData, parseKey, parseArray, syncAllData } = require('./structures/Utils');
    ...
    async set(key, value) {
        ...
        if (Array.isArray(value)) {
            if (value.length > 0) {
                ...
                if (obj.name) {
                    delete document.value[obj.name];
                    return await this.set(obj.key, document.value);
                } else {
                    return await this.delete(obj.key);
                }
            }
        }
